...
namespace Plugin.Sample.Customers.CsMigration
{
    ...
    public class ProfilesSqlContext : IEnlistmentNotification
    {
        ...
        /// </summary>
        /// <param name="enlistment">An <see cref="System.Transactions.Enlistment" /> object used to send a response to the transaction manager.</param>
        ...
        /// </summary>
        /// <param name="preparingEnlistment">A <see cref="System.Transactions.PreparingEnlistment" /> object used to send a response to the transaction manager.</param>
        ...
        /// </summary>
        /// <param name="enlistment">A <see cref="System.Transactions.Enlistment" /> object used to send a response to the transaction manager.</param>
        ...
        {
            var definitions = await GetProfileInfo().ConfigureAwait(false);

            var result = new List<ProfileDefinition>();
            foreach (var definition in definitions)
            {
                var profileDefinition = await GetProfileDefinition(definition).ConfigureAwait(false);
            ...
            using (var connection = new SqlConnection(this.ConnectionString))
            {
                ...
                foreach (DataRow row in propertiesTable.Rows)
                {
                    ...
                    var profileProperty = new ProfileProperty
                    {
                        ...
                        IsMultiValued = Convert.ToBoolean(row["IsMultiValued"], CultureInfo.InvariantCulture),
                ...
                foreach (DataRow row in propertiesTable.Rows)
                {
                    ...
                    {
                        case "MaxLength":
                            property.MaxLength = Convert.ToInt32(row["ValStr"], CultureInfo.InvariantCulture);
                            break;
                        case "MinLength":
                            property.MinLength = Convert.ToInt32(row["ValStr"], CultureInfo.InvariantCulture);
                            break;
                    }
                }
