...
namespace Plugin.Sample.Customers.CsMigration
{
    ...
    ///    </cref>
    /// </seealso> 
    [PipelineDisplayName(CustomersCsConstants.GetProfileDefinitionBlock)]
    public class GetProfileDefinitionBlock : PipelineBlock<string, IEnumerable<ProfileDefinition>, CommercePipelineExecutionContext>
    {
        ...
        public override async Task<IEnumerable<ProfileDefinition>> Run(string arg, CommercePipelineExecutionContext context)
        {
            ...
            {
                cache = await this._cachePipeline.Run(new EnvironmentCacheArgument { CacheName = cachePolicy.CacheName }, context).ConfigureAwait(false);
                if (await cache.Get(cacheKey).ConfigureAwait(false) is List<ProfileDefinition> item)
                {
                    return item;
                }
            }
            ...
            try
            {
                ...
                {
                    schema = await sqlContext.GetAllProfileDefinitions().ConfigureAwait(false);
                    if (schema != null && schema.Count > 0)
                    {
                        if (cachePolicy.AllowCaching && cache != null)
                        {
                            await cache.Set(cacheKey, new Cachable<List<ProfileDefinition>>(schema, 1), cachePolicy.GetCacheEntryOptions()).ConfigureAwait(false);
                        }
                ...
                {
                    var profileDefinition = await sqlContext.GetProfileDefinition(arg).ConfigureAwait(false);
                ...
                await context.CommerceContext.AddMessage(
                    ...
                    "EntityNotFound",
                    new object[] { arg },
                    $"Entity {arg} was not found.").ConfigureAwait(false);
                return null;
            }
            catch (Exception ex)
            {
