...
namespace Plugin.Sample.Customers.CsMigration
{
    ...
    [PipelineDisplayName(CustomersCsConstants.MapAddressesBlock)]
    public class MapAddressesBlock : PipelineBlock<Customer, Customer, CommercePipelineExecutionContext>
    {
        ...
        public override async Task<Customer> Run(Customer arg, CommercePipelineExecutionContext context)
        {
            ...
            var listProperty = profileDefinition?.Properties.FirstOrDefault(p => string.Equals(p.TypeName, "PROFILE", StringComparison.Ordinal) && p.ProfileReference.Equals("Profile Definitions.Address", StringComparison.Ordinal));
            ...
            foreach (var addressId in addressList)
            {
                try
                {
                    var addressComponent = await ComponentsHelper.AddressComponentGenerator(addressId.ToString(CultureInfo.InvariantCulture), profileDefinition, context).ConfigureAwait(false);
                ...
                catch (Exception ex)
                {
                    await context.CommerceContext.AddMessage(
                        ...
                        $"Profile address {addressId} was not found.").ConfigureAwait(false);
                    return arg;
                }
            }
