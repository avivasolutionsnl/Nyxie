...
namespace Hotcakes.Plugin.Promotions.Resolvers
{
    public class CategoryOrderLinesResolver
    {
        ...
        internal async Task<List<CartLineComponent>> Resolve(CommerceContext commerceContext, string category, bool includeSubCategories)
        {
            var allOrders = await orderResolver.Resolve(commerceContext);
        ...
        {
            return line.HasComponent<CategoryComponent>() && line.GetComponent<CategoryComponent>().IsMatch(categorySitecoreId, includeSubCategories);
        }
