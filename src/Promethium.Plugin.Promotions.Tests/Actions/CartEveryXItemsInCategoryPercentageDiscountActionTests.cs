...
namespace Promethium.Plugin.Promotions.Tests.Actions
{
    ...
    public class CartEveryXItemsInCategoryPercentageDiscountActionTests
    {
        ...
        public CartEveryXItemsInCategoryPercentageDiscountActionTests(EngineFixture engineFixture, ITestOutputHelper testOutputHelper)
        {
        ...
        public async void Should_benefit_when_category_matches()
        {
            ...
            AwardedAdjustment adjustment = line.Adjustments.Single(x => x.AwardingBlock == nameof(CartEveryXItemsInCategoryPercentageDiscountAction));
        ...
        public async void Should_benefit_multiple_times_when_within_action_limit()
        {
            ...
            AwardedAdjustment adjustment = firstLine.Adjustments.Single(x => x.AwardingBlock == nameof(CartEveryXItemsInCategoryPercentageDiscountAction));
            Assert.Equal(-20, adjustment.Adjustment.Amount);

            var secondLine = resultCart.Lines.Single(x => x.Id == "003");
            AwardedAdjustment secondAdjustment = secondLine.Adjustments.Single(x => x.AwardingBlock == nameof(CartEveryXItemsInCategoryPercentageDiscountAction));
        ...
        public async void Should_not_benefit_multiple_times_when_exceeds_action_limit()
        {
            ...
            var secondLine = resultCart.Lines.Single(x => x.Id == "003");
            Assert.DoesNotContain(secondLine.Adjustments, x => x.AwardingBlock == nameof(CartEveryXItemsInCategoryPercentageDiscountAction));
