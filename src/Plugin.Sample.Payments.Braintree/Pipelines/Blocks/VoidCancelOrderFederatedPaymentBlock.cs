        ...
        public override async Task<Order> Run(Order arg, CommercePipelineExecutionContext context)
        {
            ...
            try
            {
                ...
                if (result.IsSuccess())
                {
                    ...
                    await this.GenerateSalesActivity(order, existingPayment, context).ConfigureAwait(false);
                }
                else
                {
                    ...
                    context.Abort(
                        await context.CommerceContext.AddMessage(
                            ...
                            $"{this.Name}. Payment void failed for transaction { existingPayment.TransactionId }: { errorMessages }").ConfigureAwait(false), 
                        context);
