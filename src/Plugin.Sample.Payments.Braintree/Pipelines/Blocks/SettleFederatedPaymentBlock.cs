...
namespace Plugin.Sample.Payments.Braintree
{
    ...
    ///     </cref>
    /// </seealso>
    [PipelineDisplayName(PaymentsBraintreeConstants.SettleFederatedPaymentBlock)]
        ...
        public override async Task<SalesActivity> Run(SalesActivity arg, CommercePipelineExecutionContext context)
        {
                ...
                if (transaction.Status.ToString().Equals("authorized", StringComparison.OrdinalIgnoreCase))
                {
                        ...
                        switch (transaction.Status.ToString())
                        {
                            ...
                            default:
                                ...
                                await context.CommerceContext.AddMessage(
                                    ...
                                    $"{this.Name}. Settle payment failed for { payment.TransactionId }: { transaction.ProcessorResponseText }").ConfigureAwait(false);
                                break;
                        }
                    }
                    else
                    {
                        ...
                        await context.CommerceContext.AddMessage(
                            ...
                            "SettlePaymentFailed",
                            new object[] { payment.TransactionId },
                            $"{this.Name}. Settle payment failed for {payment.TransactionId}: {errorMessages}").ConfigureAwait(false);
                    }                    
                }
            }
            catch (BraintreeException ex)
            {
                ...
                await context.CommerceContext.AddMessage(
                    ...
                    $"{this.Name}. Settle payment failed for {payment.TransactionId}").ConfigureAwait(false);
            }
