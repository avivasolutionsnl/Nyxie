...
namespace Plugin.Sample.SideBySideUpgrade
{
    ...
    /// <seealso>
    ///     <cref>
    ///         Sitecore.Commerce.Core.PipelineBlock{Plugin.Sample.SideBySideUpgrade.MigrateListArgument,
    ///         System.Boolean, Sitecore.Commerce.Core.CommercePipelineExecutionContext}
    ///     </cref>
    /// </seealso>
    [PipelineDisplayName(SideBySideUpgradeConstants.MigrateListBlock)]
    public class MigrateListBlock : PipelineBlock<MigrateListArgument, bool, CommercePipelineExecutionContext>
    {
        ...
        public override async Task<bool> Run(MigrateListArgument arg, CommercePipelineExecutionContext context)
        {
            ...
            var count = await this._getListCountCommand.Process(context.CommerceContext, arg.ListName).ConfigureAwait(false);
            if (count > 0)
            {
            ...
            {               
                ...
                {
                    var migratedEntity = await this._migrateEntityMetadataPipeline.Run(new FindEntityArgument(typeof(CommerceEntity), id), context).ConfigureAwait(false);
