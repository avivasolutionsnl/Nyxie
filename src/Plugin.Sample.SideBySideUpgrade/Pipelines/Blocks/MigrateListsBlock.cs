...
namespace Plugin.Sample.SideBySideUpgrade
{
    ...
    ///   </cref>
    /// </seealso>
    [PipelineDisplayName(SideBySideUpgradeConstants.MigrateListsBlock)]
    public class MigrateListsBlock : PipelineBlock<CommerceEnvironment, CommerceEnvironment, CommercePipelineExecutionContext>
    {
        ...
        public override async Task<CommerceEnvironment> Run(CommerceEnvironment arg, CommercePipelineExecutionContext context)
        {
            ...
            var environment = await this._startEnvironmentPipeline.Run(arg.Name, context).ConfigureAwait(false);
            ...
            foreach (var list in migrationPolicy.ListsToMigrate)
            {
                ...
                {
                    List<string> listNames = await this._getListsNamesPipeline.Run(list.Key.Replace("*", "%"), context).ConfigureAwait(false);
