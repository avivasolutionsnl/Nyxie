...
namespace Plugin.Sample.Habitat.Pipelines.Blocks
{
    ...
    public class InitializeEnvironmentPromotionsBlock : PipelineBlock<string, string, CommercePipelineExecutionContext>
    {
        ...
        private async Task CreateCartFreeShippingPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            Promotion promotion =
                await _addPromotionPipeline.Run(
                    new AddPromotionArgument(book, "CartFreeShippingPromotion",
                    ...
                    new PromotionConditionModelArgument(
                        ...
                        new ConditionModel
                        {
                            ...
                            Properties = new List<PropertyModel>
                            {
                                new PropertyModel
                                {
                                    IsOperator = true, Name = "Operator",
                                    Value =
                                        "Sitecore.Framework.Rules.DecimalGreaterThanEqualToOperator",
                                    DisplayType =
                                        "Sitecore.Framework.Rules.IBinaryOperator`2[[System.Decimal],[System.Decimal]], Sitecore.Framework.Rules.Abstractions"
                                },
                                new PropertyModel
                                {
                                    Name = "Subtotal", Value = "100", IsOperator = false,
        ...
        private async Task CreateCartExclusive5PctOffCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            Promotion promotion =
                await _addPromotionPipeline.Run(
                    new AddPromotionArgument(book, "Cart5PctOffExclusiveCouponPromotion",
                        DateTimeOffset.UtcNow.AddDays(-2), DateTimeOffset.UtcNow.AddYears(1),
                        "5% Off Cart (Exclusive Coupon)", "5% Off Cart (Exclusive Coupon)")
                    {
                        IsExclusive = true,
                    ...
                    new PromotionConditionModelArgument(
                        ...
                        new ConditionModel
                        {
                            ...
                            Properties = new List<PropertyModel>
                            {
                                ...
                                new PropertyModel
                                {
                                    Name = "Subtotal", Value = "10", IsOperator = false,
                                    DisplayType = "System.Decimal"
                                }
                            }
                        }),
                ...
                await _addBenefitPipeline.Run(
                    new PromotionActionModelArgument(
                        ...
                        new ActionModel
                        {
                            ...
                            {
                                new PropertyModel
                                    { Name = "PercentOff", Value = "5", DisplayType = "System.Decimal" }
                            }
                        }),
                    context).ConfigureAwait(false);

            promotion = await _addPublicCouponPipeline
                              .Run(new AddPublicCouponArgument(promotion, "HABRTRNEC5P"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        private async Task CreateCartExclusive5OffCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            Promotion promotion =
                await _addPromotionPipeline.Run(
                    new AddPromotionArgument(book, "Cart5OffExclusiveCouponPromotion",
                        DateTimeOffset.UtcNow.AddDays(-3), DateTimeOffset.UtcNow.AddYears(1),
                        "$5 Off Cart (Exclusive Coupon)", "$5 Off Cart (Exclusive Coupon)")
                    {
                        IsExclusive = true,
                        DisplayName = "$5 Off Cart (Exclusive Coupon)",
                ...
                await _addBenefitPipeline.Run(
                    new PromotionActionModelArgument(
                        ...
                        new ActionModel
                        {
                            ...
                            {
                                new PropertyModel
                                    { Name = "AmountOff", Value = "5", DisplayType = "System.Decimal" }
                            }
                        }),
                    context).ConfigureAwait(false);

            promotion = await _addPublicCouponPipeline
                              .Run(new AddPublicCouponArgument(promotion, "HABRTRNEC5A"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        private async Task CreateCartExclusiveOptixCameraPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            Promotion promotion =
                await _addPromotionPipeline.Run(
                    new AddPromotionArgument(book, "CartOptixCameraExclusivePromotion",
                        DateTimeOffset.UtcNow.AddDays(-4), DateTimeOffset.UtcNow.AddYears(1),
                        "Optix Camera 50% Off Cart (Exclusive)",
                        "Optix Camera 50% Off Cart (Exclusive)")
                    {
                        IsExclusive = true,
                ...
                new PromotionActionModelArgument(
                    ...
                    new ActionModel
                    {
                        ...
                        Properties = new List<PropertyModel>
                        {
                            new PropertyModel
                            {
                                Name = "PercentOff", Value = "50", IsOperator = false,
                                DisplayType = "System.Decimal"
                            }
                        }
                    }),
        ...
        private async Task CreateCart15PctOffCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            Promotion promotion =
                await _addPromotionPipeline.Run(
                    new AddPromotionArgument(book, "Cart15PctOffCouponPromotion",
                        DateTimeOffset.UtcNow.AddDays(-5), DateTimeOffset.UtcNow.AddYears(1),
                        "15% Off Cart (Coupon)", "15% Off Cart (Coupon)")
                    {
                    ...
                    new PromotionConditionModelArgument(
                        ...
                        new ConditionModel
                        {
                            ...
                            Properties = new List<PropertyModel>
                            {
                                ...
                                new PropertyModel
                                {
                                    Name = "Subtotal", Value = "50", IsOperator = false,
                                    DisplayType = "System.Decimal"
                                }
                            }
                        }),
                ...
                await _addBenefitPipeline.Run(
                    new PromotionActionModelArgument(
                        ...
                        new ActionModel
                        {
                            ...
                            {
                                new PropertyModel
                                    { Name = "PercentOff", Value = "15", DisplayType = "System.Decimal" }
                            }
                        }),
                    context).ConfigureAwait(false);

            promotion = await _addPublicCouponPipeline
                              .Run(new AddPublicCouponArgument(promotion, "HABRTRNC15P"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        /// <returns>A <see cref="Task" /></returns>
        private async Task CreateCart10PctOffCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context,
            DateTimeOffset date)
        {
            Promotion promotion =
                await _addPromotionPipeline.Run(
                    new AddPromotionArgument(book, "Cart10PctOffCouponPromotion", date,
                        date.AddYears(1), "10% Off Cart (Coupon)", "10% Off Cart (Coupon)")
                    {
                ...
                await _addBenefitPipeline.Run(
                    new PromotionActionModelArgument(
                        ...
                        new ActionModel
                        {
                            ...
                            {
                                new PropertyModel
                                    { Name = "PercentOff", Value = "10", DisplayType = "System.Decimal" }
                            }
                        }),
                    context).ConfigureAwait(false);

            promotion = await _addPublicCouponPipeline
                              .Run(new AddPublicCouponArgument(promotion, "HABRTRNC10P"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        /// <param name="date">The date.</param>
        /// <returns>A <see cref="Task" /></returns>
        private async Task CreateCart10OffCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context,
            DateTimeOffset date)
        {
            Promotion promotion =
                await _addPromotionPipeline.Run(
                    new AddPromotionArgument(book, "Cart10OffCouponPromotion", date, date.AddYears(1),
                        "$10 Off Cart (Coupon)", "$10 Off Cart (Coupon)")
                    {
                        DisplayName = "$10 Off Cart (Coupon)",
                        Description = "$10 off Cart with subtotal of $50 or more (Coupon)"
                    },
                ...
                await _addBenefitPipeline.Run(
                    new PromotionActionModelArgument(
                        ...
                        new ActionModel
                        {
                            ...
                            {
                                new PropertyModel
                                    { Name = "AmountOff", Value = "10", DisplayType = "System.Decimal" }
                            }
                        }),
                    context).ConfigureAwait(false);

            promotion = await _addPublicCouponPipeline
                              .Run(new AddPublicCouponArgument(promotion, "HABRTRNC10A"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        private async Task CreateDisabledPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            Promotion promotion =
                await _addPromotionPipeline.Run(
                    new AddPromotionArgument(book, "DisabledPromotion", DateTimeOffset.UtcNow,
                        DateTimeOffset.UtcNow.AddYears(1), "Disabled", "Disabled")
                    {
                    ...
                    new PromotionConditionModelArgument(
                        ...
                        new ConditionModel
                        {
                            ...
                            Properties = new List<PropertyModel>
                            {
                                ...
                                new PropertyModel
                                {
                                    Name = "Subtotal", Value = "5", IsOperator = false,
                                    DisplayType = "System.Decimal"
                                }
                            }
                        }),
                    ...
                    new PromotionActionModelArgument(
                        ...
                        new ActionModel
                        {
                            ...
                            Properties = new List<PropertyModel>
                            {
                                new PropertyModel
                                {
                                    Name = "PercentOff", Value = "100", DisplayType = "System.Decimal"
                                }
                            }
                        }),
                    context).ConfigureAwait(false);
        ...
        private async Task CreateLineTouchScreenPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            Promotion promotion =
                await _addPromotionPipeline.Run(
                    new AddPromotionArgument(book, "LineHabitat34withTouchScreenPromotion",
                        DateTimeOffset.UtcNow.AddDays(-1), DateTimeOffset.UtcNow.AddYears(1),
                        "Habitat Touch Screen 50% Off", "Habitat Touch Screen 50% Off")
                    {
                        DisplayName = "Habitat Touch Screen 50% Off",
                        Description =
                            "50% off the Habitat 34.0 Cubic Refrigerator with Touchscreen item"
                    },
                    context).ConfigureAwait(false);
                ...
                new PromotionActionModelArgument(
                    ...
                    new ActionModel
                    {
                        ...
                        Properties = new List<PropertyModel>
                        {
                            ...
                            new PropertyModel
                            {
                                Name = "TargetItemId", Value = "Habitat_Master|6042588|",
                                IsOperator = false, DisplayType = "System.String"
                            }
                        }
                    }),
                context).ConfigureAwait(false);
        ...
        private async Task CreateLineLaptopPricePromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            Promotion promotion =
                await _addPromotionPipeline.Run(
                    new AddPromotionArgument(book, "LineHabitatLaptopPricePromotion",
                        DateTimeOffset.UtcNow.AddDays(-1), DateTimeOffset.UtcNow.AddYears(1),
                        "Pay only $5", "Pay only $5")
                    {
                ...
                new PromotionActionModelArgument(
                    ...
                    new ActionModel
                    {
                        ...
                        Properties = new List<PropertyModel>
                        {
                            new PropertyModel
                            {
                                Name = "SellPrice", Value = "5", IsOperator = false,
                                DisplayType = "System.Decimal"
                            },
                            new PropertyModel
                            {
                                Name = "TargetItemId", Value = "Habitat_Master|6042178|",
                                IsOperator = false, DisplayType = "System.String"
                            }
                        }
                    }),
            ...
            promotion = await _addPublicCouponPipeline
                              .Run(new AddPublicCouponArgument(promotion, "HABSELLPRICE"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        private async Task CreateLineTouchScreen5OffPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            Promotion promotion =
                await _addPromotionPipeline.Run(
                    new AddPromotionArgument(book, "LineHabitat34withTouchScreen5OffPromotion",
                        DateTimeOffset.UtcNow.AddDays(-2), DateTimeOffset.UtcNow.AddYears(1),
                        "Habitat Touch Screen $5 Off Item", "Habitat Touch Screen $5 Off Item")
                    {
                        DisplayName = "Habitat Touch Screen $5 Off",
                        Description =
                            "$5 off the Habitat 34.0 Cubic Refrigerator with Touchscreen item"
                    },
                    context).ConfigureAwait(false);
                ...
                new PromotionActionModelArgument(
                    ...
                    new ActionModel
                    {
                        ...
                        Properties = new List<PropertyModel>
                        {
                            new PropertyModel
                            {
                                Name = "AmountOff", Value = "5", IsOperator = false,
                                DisplayType = "System.Decimal"
                            },
                            new PropertyModel
                            {
        ...
        private async Task CreateLineExclusiveMiraLaptopPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            Promotion promotion =
                await _addPromotionPipeline.Run(
                    new AddPromotionArgument(book, "LineMiraLaptopExclusivePromotion",
                        DateTimeOffset.UtcNow.AddDays(-2), DateTimeOffset.UtcNow.AddYears(1),
                        "Mira Laptop 50% Off Item (Exclusive)", "Mira Laptop 50% Off Item (Exclusive)")
                    {
                ...
                new PromotionActionModelArgument(
                    ...
                    new ActionModel
                    {
                        ...
                        Properties = new List<PropertyModel>
                        {
                            ...
                            new PropertyModel
                            {
                                Name = "TargetItemId", Value = "Habitat_Master|6042179|",
                                IsOperator = false, DisplayType = "System.String"
                            }
                        }
                    }),
        ...
        private async Task CreateLineExclusive20PctOffCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            Promotion promotion =
                await _addPromotionPipeline.Run(
                    new AddPromotionArgument(book, "Line20PctOffExclusiveCouponPromotion",
                        DateTimeOffset.UtcNow.AddDays(-3), DateTimeOffset.UtcNow.AddYears(1),
                        "20% Off Item (Exclusive Coupon)", "20% Off Item (Exclusive Coupon)")
                    {
                        IsExclusive = true,
                        DisplayName = "20% Off Item (Exclusive Coupon)",
                        Description =
                            "20% off any item with subtotal of $50 or more (Exclusive Coupon)"
                    },
                    context).ConfigureAwait(false);
                    ...
                    new PromotionConditionModelArgument(
                        ...
                        new ConditionModel
                        {
                            ...
                            Properties = new List<PropertyModel>
                            {
                                ...
                                new PropertyModel
                                {
                                    Name = "Subtotal", Value = "25", IsOperator = false,
                                    DisplayType = "System.Decimal"
                                }
                            }
                        }),
                    ...
                    new PromotionActionModelArgument(
                        ...
                        new ActionModel
                        {
                            ...
                            Properties = new List<PropertyModel>
                            {
                                new PropertyModel
                                {
                                    Name = "PercentOff", Value = "20", DisplayType = "System.Decimal"
                                },
                                new PropertyModel
                                {
                                    IsOperator = true, Name = "Operator",
            ...
            promotion = await _addPublicCouponPipeline
                              .Run(new AddPublicCouponArgument(promotion, "HABRTRNEL20P"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        private async Task CreateLineExclusive20OffCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            Promotion promotion =
                await _addPromotionPipeline.Run(
                    new AddPromotionArgument(book, "Line20OffExclusiveCouponPromotion",
                        DateTimeOffset.UtcNow.AddDays(-4), DateTimeOffset.UtcNow.AddYears(1),
                        "$20 Off Item (Exclusive Coupon)", "$20 Off Item (Exclusive Coupon)")
                    {
                        IsExclusive = true,
                ...
                await _addBenefitPipeline.Run(
                    new PromotionActionModelArgument(
                        ...
                        new ActionModel
                        {
                            ...
                            {
                                new PropertyModel
                                    { Name = "AmountOff", Value = "20", DisplayType = "System.Decimal" },
                                new PropertyModel
                                {
                                    IsOperator = true, Name = "Operator",
                                    Value = "Sitecore.Framework.Rules.DecimalGreaterThanEqualToOperator",
            ...
            promotion = await _addPublicCouponPipeline
                              .Run(new AddPublicCouponArgument(promotion, "HABRTRNEL20A"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        private async Task CreateLine5PctOffCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            Promotion promotion =
                await _addPromotionPipeline.Run(
                    new AddPromotionArgument(book, "Line5PctOffCouponPromotion",
                        DateTimeOffset.UtcNow.AddDays(-5), DateTimeOffset.UtcNow.AddYears(1),
                        "5% Off Item (Coupon)", "5% Off Item (Coupon)")
                    {
            ...
            promotion = await _addPublicCouponPipeline
                              .Run(new AddPublicCouponArgument(promotion, "HABRTRNL5P"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        private async Task CreateLine5OffCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            Promotion promotion =
                await _addPromotionPipeline.Run(
                    new AddPromotionArgument(book, "Line5OffCouponPromotion",
                        DateTimeOffset.UtcNow.AddDays(-6), DateTimeOffset.UtcNow.AddYears(1),
                        "$5 Off Item (Coupon)", "$5 Off Item (Coupon)")
                    {
                        DisplayName = "$5 Off Item (Coupon)",
                        Description = "$5 off any item with subtotal of $10 or more (Coupon)"
            ...
            promotion = await _addPublicCouponPipeline
                              .Run(new AddPublicCouponArgument(promotion, "HABRTRNL5A"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
    }
